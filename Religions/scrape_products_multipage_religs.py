#-*- coding: utf-8 -*-
"""
## Parse pages.htm into tables attributes.tsv and edges.tsv
## Modified 2016-10-10
## Nathaniel Porter
...
def get_attributes(page, out):
    ...
    asin, title, author, publisher, prdtype, salesrank, isbn10, isbn13, authorcount, kwstring = ('','','','','O',0,'','',0,'')
    lines = page.split('\n')
    for line in lines:        
        #asin
        if line.startswith(r'        <div data-a-carousel-options='):
    ...
    if asin != '':
        out.write('\n')
        try:
            out.write(asin+'\t'+title+'\t'+author+'\t'+publisher+'\t'+prdtype+'\t'+str(salesrank)+'\t'+isbn10+'\t'+isbn13+'\t'+str(authorcount)+'\t'+kwstring)
            ...
            print('\n'+asin+'\t'+title+'\t'+author+'\t'+publisher+'\t'+prdtype+'\t'+str(salesrank)+'\t'+isbn10+'\t'+isbn13+'\t'+str(authorcount)+'\t'+kwstring)
...
mainDir = r'\Dissertation 2017\Data\Archive\Religions' #main working directory
inSubDir = '' #location of input files
...
with open('edges.tsv', 'w') as outfile:
    ...
    for fullfile in filelist:
		...
		for page in subpages(f):
			get_edges(page, outfile)       

##Attributes of products
with open('attributes.tsv', 'w') as outfile:
    #output file header outfile.write('asin\ttitle\tauthor\tpublisher\tprdtype\tsalesrank\tisbn10\tisbn13\tauthorcount\tkwstring')
    for fullfile in filelist:
        with open(fullfile, 'r') as f:
            for page in subpages(f):
